...
WORD make_code(WORD endf, WORD exbus, WORD ex, WORD alu, WORD br, WORD wr, WORD regw_en, WORD r_latch,
	WORD stfr, WORD f_latch_en, WORD fb_sel, WORD din_latch, WORD misc){
...
WORD make_code_imm8(WORD endf, WORD imm8, WORD fb_sel){
...
WORD FETCH_BYTE_CPOUT ;	//	CP出力→CP+
WORD FETCH_BYTE_READDATA ;	//	1バイトフェッチ
WORD CPOUT_INC ;	//	CP出力→CP+

WORD WRITE_BYTE ;
WORD SPINC ;
WORD SPDEC ;
WORD CPOUT ;
WORD DATAIN_BUFF1_CPINC ;
WORD DATAIN_BUFF2_CPINC ;
WORD ENDMARK ;
...
/*
WORD mem_read_op(BYTE addr_out, BYTE read_buff){

}

WORD mem_write_op(BYTE addr_out, BYTE out_buff){
...
/*
	fp=fopen("298uROM.mem", "w") ;
	for(i=0 ; i<1024 ; i++){
		fprintf(fp, "%x\n", mem[i]) ;
	}
	fclose(fp) ;

	fp=fopen("298uROMTBL.TXT", "w") ;
	fprintf(fp, "v2.0 raw\x0a") ;
	for(i=0 ; i<tbl_idx ; i++){
